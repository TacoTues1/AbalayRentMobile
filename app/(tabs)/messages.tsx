  ...
  const loadConversations = async (userId: string, role: string) => {
    ...
    if (normalizedRole === "tenant") {
      for (const landlordId of allowedSet) {
        const exists = existingConvs.find(
      ...
      if (occupancies) {
        for (const occ of occupancies) {
          const exists = existingConvs.find(
            (c) => c.landlord_id === userId && c.tenant_id === occ.tenant_id,
          );
          if (!exists) {
            const { data: newConv } = await supabase
              .from("conversations")
    ...
    const enrichedConvs = await Promise.all(
      existingConvs.map(async (conv: any) => {
        const isLandlord = conv.landlord_id === userId;
        const otherUserId = isLandlord ? conv.tenant_id : conv.landlord_id;
        ...
        <View style={{ flex: 1, marginBottom: 100 }}>
          ...
          >
            ...
            <View style={styles.chatHeaderUser}>
              ...
              <View style={{ flex: 1 }}>
                <Text
                  ...
                  numberOfLines={1}
                >
                  {selectedConv.otherUser?.first_name}{" "}
                  {selectedConv.otherUser?.last_name}
                </Text>
            ...
            <View style={{ flexDirection: "row", gap: 4 }}>
              <TouchableOpacity
                onPress={() => loadSharedMedia(selectedConv.id)}
                style={[
              ...
              <TouchableOpacity
                onPress={() => deleteConversation(selectedConv.id)}
                style={[
                  styles.chatHeaderAction,
                  { backgroundColor: isDark ? colors.card : "#f3f4f6" },
                ]}
