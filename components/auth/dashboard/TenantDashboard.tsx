...
function extractCoordinates(link: string | null | undefined) {
  if (!link) return null;
  const atMatch = link.match(/@(-?\d+\.\d+),(-?\d+\.\d+)/);
  const qMatch = link.match(/[?&]q=(-?\d+\.\d+),(-?\d+\.\d+)/);
  const placeMatch = link.match(/place\/(-?\d+\.\d+),(-?\d+\.\d+)/);
  const genericPairMatch = link.match(
    /(-?\d{1,2}\.\d+)\s*,\s*(-?\d{1,3}\.\d+)/,
  );
...
export default function TenantDashboard({ session, profile }: any) {
  ...
  }, [properties, guestFavorites, nearbyProperties, cityProperties, topRated]);
  ...
  const loadPropertiesData = async () => {
    try {
      ...
      // --- Location-based sections (matches web) ---
      try {
        ...
        if (status === "granted") {
          ...
          try {
            ...
            // Available in [City] — filter properties by city
            if (city) {
  ...
  const requestEndOccupancy = async () => {
    ...
    try {
      ...
      if ((pendingPaymentCount || 0) > 0) {
        Alert.alert(
          "Warning",
          "You have pending payments. Please settle all outstanding bills as soon as possible.",
        );
      }
  ...
  const filteredProperties = properties
    .filter((p: any) => {
      ...
      if (noOccupancyMaxPrice !== null && p.price > noOccupancyMaxPrice)
        return false;
      if (
      ...
      if (
        ...
        (propertyStats[p.id]?.avg_rating || 0) < noOccupancyMinRating
      )
        return false;
      if (
  ...
  // --- HELPER: See All Card (matches web's renderSeeAllCard) ---
  const renderSeeAllCard = (items: any[], onPress: () => void) => {
    const defaultImg =
      "https://images.unsplash.com/photo-1560518884-ce5882228f44?w=500&q=80";
    const getImg = (idx: number) => {
      const item = items[idx % items.length];
      return item?.images?.[0] || defaultImg;
    };
                    ...
                    <>
                      <View style={styles.gridActions}>
                        ...
                        {occupancy.contract_url ? (
                          <TouchableOpacity
                            style={[styles.gridBtn, styles.btnBlack]}
                            onPress={() =>
            ...
            {showRecentPaymentsSkeleton ? (
              ...
              >
                ...
                <Text style={styles.noteText}>
                  Note: Please ensure all electricity, water and wifi bills are
                  paid before the due date. The landlord is not liable for late
                  payments.
                </Text>
              </View>
            )}
          ...
          >
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity
                style={[
                  styles.confirmBtn,
                  { backgroundColor: isDark ? "#eab308" : "#111" },
                ]}
